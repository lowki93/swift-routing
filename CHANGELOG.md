...
- `printRouter(trigger:)`/`printRouterOnChange()` view modifiers to print the full router hierarchy to the console for debugging (`DEBUG` builds only) -- stack routers print their whole navigation stack (root through current route), not just the current route
