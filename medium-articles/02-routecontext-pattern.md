...
*Part 2 of the swift-routing series. If you're new here, start with [Part 1: Type-Safe Navigation in SwiftUI with swift-routing](https://medium.com/@budainkevin/stop-fighting-swiftui-navigation-a-type-safe-approach-with-swift-routing-7cbd328f0270).*

Navigation in SwiftUI is a one-way street by default. You push a screen, it does something, and then… You have no clean way to return the result.
...
Here's a common scenario. You have a list of users. You present a picker screen. The user selects someone. You need to restore that selection to the parent.
...
- `router.context(_:)`: fires context, but no navigation
- `router.terminate(_:)`: fires context and dismiss sheet or pop a right screen.
...
.routerContext(ProfileUpdatedContext.self) { _ in
    self?.refreshBadge()
}
...
The problem appears when the communication is transient. A user picker doesn't belong in your app's global state. It runs, returns a value, and disappears. Injecting a `@Observable` for that means creating something that exists only to be discarded, and whose lifetime you have to manage manually.
