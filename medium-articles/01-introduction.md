...
A few months ago, while working at Sorare, I ran into a problem that felt minor at first — and then became unavoidable.

The app was organized into feature frameworks: one for the marketplace, one for the game, one for the card, and more... Each feature owned its screens. Clean, modular, scalable. Except for one thing: to navigate from Feature A to a screen in Feature B, Feature A had to import Feature B. And Feature B sometimes imported Feature A back. Circular dependencies started appearing. The module graph turned into a tangle.

The solution wasn’t a navigation library — it was a constraint: routes had to live outside the views they represent. Once routes became standalone values, features could navigate to each other without importing one another. The dependency problem disappeared.
...
Now imagine five features, each with its own destinations. A few problems compound fast:

**1. Routes are strongly-typed or weakly typed.** Passing a `String` to identify a destination is fragile. There’s no compiler guarantee that the destination exists or that you’re passing the right data.
...
You define an enum that represents every destination in your application:
...
In a modular app, this enum lives in a shared module — a `Route` or `AppRoutes` target — that every feature can import without importing each other. Feature A says `router.push(AppRoute.details)`` without knowing anything about the checkout feature’s internals.
...
This separation is what makes the modular architecture work: the `AppRoutes` module holds the enum, the `App` or composition root holds the `RouteDestination` conformance, and feature modules stay isolated.
...
```swift
// Always opens as a sheet, wherever this is called.
router.route(AppRoute.settings) 
```
...
```swift
// Package.swift
.package(url: "https://github.com/lowki93/swift-routing", from: "0.2.0")
```
